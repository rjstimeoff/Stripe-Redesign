...
const frag = /* glsl */`
  ...
  void main() {
    ...
    // page-space y: fixed position on the page regardless of scroll
    float y = uv.y / uTotalViewports + uScrollProgress * (1.0 - 1.0 / uTotalViewports);
    ...
    // scroll reveal: ribbon draws on from top as you scroll
    float reveal = smoothstep(y, y + 0.1, uScrollProgress - 0.07);

    gl_FragColor = vec4(color, alpha * sideFade * reveal);
  }
`;
    ...
    import("ogl").then(({ Renderer, Program, Mesh, Triangle }) => {
      ...
      const program = new Program(gl, {
        ...
        uniforms: {
          ...
          uTotalViewports: { value: section.offsetHeight / window.innerHeight },
        },
      });
      ...
      const handleResize = () => {
        ...
        program.uniforms.uTotalViewports.value = section.offsetHeight / window.innerHeight;
      };
      handleResize();
      window.addEventListener("resize", handleResize);

      const unsubscribe = scrollYProgress.on("change", (v) => {
        program.uniforms.uScrollProgress.value = v;
      });
